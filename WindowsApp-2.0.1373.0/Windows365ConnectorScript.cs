...
public class Windows365ConnectorScript : ConnectorScriptBase
{
    ...
    private void HandleSignInFlow()
    {
        ...
        {
            if (!TryResetToUseAnotherAccount(TimeoutSeconds, globalWaitInSeconds))
        ...
        _cancellationToken.ThrowIfCancellationRequested();
        var editEnterYourEmail = authWindow.FindAutomationElementByXPathOrInformation("//Group/Edit[@Name=\"Enter your email or phone\"][@AutomationId=\"i0116\"]", "i0116",
                                                                                      "", "Enter your email or phone", "Edit",
                                                                                      globalWaitInSeconds);
        ...
        if (!RedirectNext(authWindow, globalWaitInSeconds))
        {
            _cancellationToken.ThrowIfCancellationRequested();
        ...
        var staySigned = authWindow.FindAutomationElementByXPathOrInformation($"//Group/Button[@Name=\"No\"][@AutomationId=\"idBtn_Back\"]",
        ...
        var reviewInfoGood = FindAutomationElementByXPathOrInformation(
            automationId: "", className: "", name: "Looks good", controlType: "Button",
            xpath: "/Window[@ClassName=\"ApplicationFrameWindow\"]" +
                   "/Window[@ClassName=\"Windows.UI.Core.CoreWindow\"]" +
                   "/Pane[@ClassName=\"WebView\"][@AutomationId=\"webView\"]" +
                   "/Pane" +
                   "/Group" +
                   "/Pane" +
                   "/Button[@Name=\"Looks good\"]",
            timeout: globalWaitInSeconds, continueOnError: true);

        reviewInfoGood?.Click();
    ...
    private bool RedirectNext(IAutomationElement authWindow, int globalWaitInSeconds)
    {
        LogInfo("Redirect check");

        var buttonNext = authWindow.FindAutomationElementByXPathOrInformation("//Group/Button[@Name=\"Next\"][@AutomationId=\"idSubmit_ProofUp_Redirect\"]",
                                                                              "idSubmit_ProofUp_Redirect", "", "Next", "Button",
                                                                              globalWaitInSeconds, continueOnError: true);

        if (buttonNext != null)
        {
            LogInfo("Redirect confirmed");
            buttonNext.Click(forceFocus: false);
            Wait(2);

            LogInfo("Use account");
            var useAccount = authWindow.FindAutomationElementByXPathOrInformation($"//Group/List/ListItem/Button[contains(@Name, '{RuntimeEmail}')]",
                                                                                      "", "", "", "Button",
                                                                                      globalWaitInSeconds, continueOnError: true);
    ...
    private void SelectCloudPc()
    {
        ...
        while (DateTime.UtcNow < stopAt && !devicesButtonFound)
        {
            ...
            try
            {
                ...
                if (devicesWindow != null)
                {
                    ...
                    LogInfo("Looking for devices button");
        ...
        _cancellationToken.ThrowIfCancellationRequested();
        var connectCards = devicesWindow.FindAllAutomationElementByXPathOrInformation($"//Document/Group/Group/Group/Group/Button[@Name=\"Connect to {CloudPcTitle}. Press Enter to connect.\"]",
        ...
        var connections = FindAllAutomationElementByXPathOrInformation("/Window[@ClassName=\"TscShellContainerClass\"][@Name=\"" + CloudPcTitle + "*\"]",
                                                                       ...
                                                                       globalWaitInSeconds,
                                                                       continueOnError: true);
        connectCards.First().Click(forceFocus: false);
    ...
    private void PostConnectionEstablished()
    {
        ...
        if (!TryResetToUseAnotherAccount(TimeoutSeconds, globalWaitInSeconds))
            LogWarning("Could not reset the state after connection");
    }
    ...
    private bool TryResetToUseAnotherAccount(int timeoutSeconds, int waitInSeconds)
    {
        LogDebug("Resetting to use another account state...");
        ...
        while (DateTime.UtcNow < stopAt)
        {
            ...
            try
            {
                ...
                if (accountProbe != null)
                {
                    ...
                    _cancellationToken.ThrowIfCancellationRequested();
                    var signOutBtn = devicesWindow.FindAutomationElementByXPathOrInformation("//Group/Window/Button[contains(@Name, 'Sign out ')]", "", "", "Sign out*", "Button", waitInSeconds);
                ...
                else if (IsItInSignInState(waitInSeconds))
                {
                    var signInButton = _windowElement.FindAutomationElementByXPathOrInformation("//Group/Button[@Name=\"Sign in\"]", "", "", "Sign in", "Button", waitInSeconds);
